...
import gestalt.candidates.glsl.ShaderMaterial;
import gestalt.candidates.rendertotexture.JoglTexCreatorFBO_RGBA32Float;
...
public class GPGPUCAWater
    ...
    private final JoglTexturePlugin _myInputEnergyMap;
    ...
    public GPGPUCAWater(ShaderManager theShaderManager,
        ...
        _myShaderManager.attachFragmentShader(_myShaderProgram, GlobalPrefs.getInstance().getStream(theParticleFragShader));

        /* create water shader drawer */
        _myWaterDrawerShader = _myShaderManager.createShaderProgram();
        _myShaderManager.attachFragmentShader(_myWaterDrawerShader, GlobalPrefs.getInstance().getStream(theWaterDrawerShader));
    }

    public void draw(GLContext theRenderContext) {
        final GL gl = ((JoglGLContext)theRenderContext).gl;
        final GLU glu = ((JoglGLContext)theRenderContext).glu;
